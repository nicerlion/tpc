...
class WC_Checkout {
	...
	protected function validate_posted_data( &$data, &$errors ) {
		...
		$tpc_errorv2 = $tpc_error;
		if (isset($_POST["hidden_upsell"]) and $_POST["hidden_upsell"] == "yes" and $_POST["upsell_product"] != "" and $tpc_error == "not") {
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				if ($_POST["upsell_product"] == $cart_item["product_id"]) {
					$tpc_cart = "yes";
				}
			}
			if ($tpc_cart == "not") {
				WC()->cart->add_to_cart($_POST["upsell_product"]);
			}
			$tpc_error = "yes";
		}
		if (isset($_POST["hidden_upsellv2"]) and $_POST["hidden_upsellv2"] == "yes" and $_POST["id_hidden_upsellv2"] != "" and $tpc_errorv2 == "not") {
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				if ($_POST["id_hidden_upsellv2"] == $cart_item["product_id"]) {
					$tpc_cart = "yes";
				}
			}
			if ($tpc_cart == "not") {
				WC()->cart->add_to_cart($_POST["id_hidden_upsellv2"]);
			}
			$tpc_errorv2 = "yes";
		}
	}
			...
			if ( empty( $shipping_country ) ) {
				...
				foreach ( WC()->shipping->get_packages() as $i => $package ) {
					if ( ! isset( $chosen_shipping_methods[ $i ], $package['rates'][ $chosen_shipping_methods[ $i ] ] ) ) {
						$errors->add( 'shipping', __( 'Sorry, you have chosen a country that is outside of our shipping zone. We currently only ship to the United States and Canada.', 'woocommerce' ) );
					}
				}
			}
		}
